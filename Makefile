...
next:
	docker run -e NEXT_PUBLIC_URL -e DENY_ROBOTS_TXT -e NEXT_PUBLIC_REMOVE_ADVERTS -e NEXT_PUBLIC_INSTANCESKV_URL -e OPENGRAPH_URL -v $(shell pwd):/app -w /app --rm -t node:$(shell cat next/.nvmrc | tr -d 'v')-alpine sh -c 'cd next && npm ci && npm run build'
	cp -a next/out/. www/

  ...
  const columns: ColumnDef<Instance>[] = [
    ...
    },
    {
      ...
      cell: info => {
        ...
        if (value === 'variable') {
              ...
              <span>
                <abbr title="For T2 instances, the 100% unit represents a High Frequency Intel Xeon Processors with Turbo up to 3.3GHz.">
                  <a href="https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/burstable-performance-instances.html" target="_blank">
                    Base performance: {`${(basePerformance * 100).toFixed(1)}%`}
                  </a>
                </abbr>
              </span>
            );
          ...
          return (
            <span>
              <a href="https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/concepts_micro_instances.html" target="_blank">
                Burstable
              </a>
            </span>
          );
    ...
    },
    {
      ...
      cell: info => {
        ...
        return price ? `$${Number(price).toFixed(4)} hourly` : 'unavailable';
      },
    },
    {
      accessorKey: 'pricing',
            ...
            {table.getHeaderGroups().map(headerGroup => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map(header => (
                  <th key={header.id} className="sticky top-0 bg-gray-50">
                    {flexRender(

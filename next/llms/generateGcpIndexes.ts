...
function calculatePrice(instance: GCPInstance) {
    const usEast1 = instance.pricing["us-east1"];
    if (!usEast1) {
        return "N/A";
    }
    if (usEast1.linux) {
...
export const gcpIndexes = [
    ...
    },
    {
        ...
        slug: "over-512gb-ram",
        filter: (instance: GCPInstance) => instance.memory > 512,
    },
...

    {
        ...
        slug: "under-4-vcpu",
        filter: (instance: GCPInstance) => instance.vCPU < 4,
    },
    {
    ...
    },
    {
        ...
        slug: "over-128-vcpu",
        filter: (instance: GCPInstance) => instance.vCPU > 128,
    },
    ...
    },
    {
        ...
        filter: (instance: GCPInstance) => Number(calculatePrice(instance)) > 8,
    },
];
...
export async function generateGcpIndexes(pathPrefix: string) {
    const instances = await gcpInstances;
    const buckets = new Map<string, GCPInstance[]>();
    for (const instance of instances) {
        for (const index of gcpIndexes) {
    ...
    for (const [slug, instances] of buckets.entries()) {
        const markdown = generateIndexMarkdown(
            pathPrefix,
            gcpIndexes.find((i) => i.slug === slug)!.name,
            instances,
        );

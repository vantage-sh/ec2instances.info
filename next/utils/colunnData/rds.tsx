...
export function makePrettyNames<V>(
    ...
    return [
        ...
        makeColumnOption(
            "ebs-throughput",
            t?.("columns.rds.ebsThroughput") ?? "EBS Throughput",
        ),
        makeColumnOption(
            "physical_processor",
            t?.("columns.common.processor") ?? "Processor",
        ),
        ...
        makeColumnOption(
            "cost-ondemand-14",
            t?.("columns.rds.postgresql") ?? "PostgreSQL",
        ),
        makeColumnOption(
            "cost-reserved-14t",
            t?.("columns.rds.postgresqlReserved") ?? "PostgreSQL Reserved Cost",
        ),
        makeColumnOption(
            "cost-ondemand-2",
            t?.("columns.rds.mysqlOnDemand") ?? "MySQL On Demand Cost",
        ),
        makeColumnOption(
            "cost-reserved-2",
            t?.("columns.rds.mysqlReserved") ?? "MySQL Reserved Cost",
        ),
        makeColumnOption(
            "cost-ondemand-10",
            t?.("columns.rds.sqlExpressOnDemand") ??
                "SQL Server Expresss On Demand Cost",
        ),
        makeColumnOption(
            "cost-reserved-10",
            t?.("columns.rds.sqlExpressReserved") ??
                "SQL Server Expresss Reserved Cost",
        ),
        makeColumnOption(
            "cost-ondemand-11",
            t?.("columns.rds.sqlWebOnDemand") ??
                "SQL Server Web On Demand Cost",
        ),
        makeColumnOption(
            "cost-reserved-11",
            t?.("columns.rds.sqlWebReserved") ?? "SQL Server Web Reserved Cost",
        ),
        makeColumnOption(
            "cost-ondemand-12",
            t?.("columns.rds.sqlStdOnDemand") ??
                "SQL Server Standard On Demand Cost",
        ),
        makeColumnOption(
            "cost-reserved-12",
            t?.("columns.rds.sqlStdReserved") ??
                "SQL Server Standard Reserved Cost",
        ),
        makeColumnOption(
            "cost-ondemand-15",
            t?.("columns.rds.sqlEntOnDemand") ??
                "SQL Server Enterprise On Demand Cost",
        ),
        makeColumnOption(
            "cost-reserved-15",
            t?.("columns.rds.sqlEntReserved") ??
                "SQL Server Enterprise Reserved Cost",
        ),
        makeColumnOption(
            "cost-ondemand-21",
            t?.("columns.rds.auroraOnDemand") ??
                "Aurora Postgres & MySQL On Demand Cost",
        ),
        makeColumnOption(
            "cost-reserved-21",
            t?.("columns.rds.auroraReserved") ??
                "Aurora Postgres & MySQL Reserved Cost",
        ),
        makeColumnOption(
            "cost-ondemand-211",
            t?.("columns.rds.auroraIoOnDemand") ??
                "Aurora I/O Optimized On Demand Cost",
        ),
        makeColumnOption(
            "cost-ondemand-18",
            t?.("columns.rds.mariadbOnDemand") ?? "MariaDB On Demand Cost",
        ),
        makeColumnOption(
            "cost-reserved-18",
            t?.("columns.rds.mariadbReserved") ?? "MariaDB Reserved Cost",
        ),
        makeColumnOption(
            "cost-ondemand-5",
            t?.("columns.rds.oracleOnDemand") ??
                "Oracle Enterprise BYOL On Demand Cost",
        ),
        makeColumnOption(
            "cost-reserved-5",
            t?.("columns.rds.oracleReserved") ??
...
export const columnsGen = (
    ...
    return [
        ...
        {
            header:
                t?.("columns.rds.postgresqlReserved") ??
                "PostgreSQL Reserved Cost",
        ...
        {
            header:
                t?.("columns.rds.sqlWebReserved") ??
                "SQL Server Web Reserved Cost",
            id: "cost-reserved-11",
            accessorKey: "pricing",
            ...getPricingSorter(
                selectedRegion,
        ...
        },
        {
            header:
                t?.("columns.rds.oracleOnDemand") ??
                "Oracle Enterprise On Demand Cost",
            id: "cost-ondemand-5",
            accessorKey: "pricing",
            ...getPricingSorter(
                selectedRegion,
        ...
        },
        {
            header:
                t?.("columns.rds.oracleReserved") ??
                "Oracle Enterprise Reserved Cost",
            id: "cost-reserved-5",
            accessorKey: "pricing",
            ...getPricingSorter(
                selectedRegion,

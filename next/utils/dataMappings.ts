...
const OTHER_SAVINGS_GROUP = "Other Savings";
const INSTANCE_SAVINGS_GROUP = "Instance Savings Plans";

const sharedReservedTermOptions: ReservedTermOption[] = [
    {
        value: "yrTerm1Standard.noUpfront",
        label: "1-year - No Upfront",
        group: OTHER_SAVINGS_GROUP,
    },
    {
...
export const reservedTermOptions = (
    ...
    const instanceExtras: ReservedTermOption[] = [];
    const otherExtras: ReservedTermOption[] = [];
    for (const savingsPlan of savingsPlanSupported) {
        ...
        if (option.group === INSTANCE_SAVINGS_GROUP) {
            instanceExtras.push(option);
        } else {
            otherExtras.push(option);
        }

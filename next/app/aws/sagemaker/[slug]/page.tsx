...
function generateDescription(
    instance: Instance,
    ondemandCost: string | number | undefined,
) {
...
export async function generateMetadata({
    ...
    return {
        ...
        openGraph: {
            images: [
                urlInject`${"/aws/sagemaker/" + instance.instance_type + ".png"}`,
            ],
        },
    };
}
...
export default async function Page({
    ...
    const [prefix, itype] = instance.instance_type.split(".", 3);
    const allOfInstanceType = instances
        .filter((i) => i.instance_type.startsWith(`${prefix}.${itype}.`))
        .map((i) => ({
